...
Quick start:
    ...
    svc = SvcClient(token="...")            # token is the live API token
...
which returns an iterator of (event, data) tuples decoded from SSE.
...
- If `token` is omitted, the SDK falls back to $ANET_TOKEN, then tries the
  on-disk file at $HOME/.anet/api_token; raises AuthMissingError if none found.
"""
...
class SvcAPIError(SvcError):
    ...
    def __init__(self, status: int, message: str = "", errors: list[str] | None = None,
        ...
        if errors:
            ...
            super().__init__(f"daemon HTTP {status}: {message or body[:200].decode('utf-8', 'replace')}")
...
def _resolve_token(explicit: str | None) -> str | None:
    ...
    if env:
        return env
    # Fall back to on-disk file written by `anet daemon` first-run.
...
def _norm_paths(paths: Iterable[Any]) -> list[dict]:
    """Accept ['/echo'] or [{'prefix':'/echo','methods':['POST']}] and normalize."""
    out: list[dict] = []
    for p in paths:
        if isinstance(p, str):
            out.append({"prefix": p})
...
def _build_cost_model(*, free: bool, per_call: int | None, per_kb: int | None,
                      per_minute: int | None, deposit: int | None,
                      override: Mapping | None) -> dict | None:
...
class SvcClient:
    ...
    def _check(self, r: httpx.Response) -> Any:
        if r.status_code >= 400:
            ...
            try:
                ...
                pass
            raise SvcAPIError(r.status_code, message=message, errors=errors, body=r.content)
        if not r.content:
            return None
    ...
    def register(
        ...
        **extra: Any,
        ...
        """Register a local service so other peers can discover & call it.

        At least one cost dimension is required unless ``free=True`` is passed
        (CP4). For non-localhost endpoints the daemon's ``svc_remote_allowlist``
        and the per-service ``remote_hosts`` list must both opt in (CP3).
        """
        cm = _build_cost_model(
            free=free, per_call=per_call, per_kb=per_kb,
            per_minute=per_minute, deposit=deposit, override=cost_model,
        )
        body: dict[str, Any] = {
            "name": name,
            "endpoint": endpoint,
    ...
    def discover(
        ...
        *,
        ...
        """Look up services. Pass ``peer_id`` for direct P2P pull (returns the
        provider's full service registry envelope) or ``skill`` for ANS-backed
        capability search (returns ``[{peer_id, owner_did, services:[...]}]``).
        """
    ...
    def call(
        ...
        *,
        ...
        """Make one rr (request/response) call to a remote service.

        Returns the daemon envelope ``{status, headers, body, error?, cost?}``.
        ``body`` may be a JSON-serialisable object, a string, or bytes.
        Set ``passthrough_status=True`` (CP5) to map the upstream HTTP status
        onto the outer HTTP response so ``curl --fail``-style retry works.
        """
        req: dict[str, Any] = {
            "peer_id": peer_id,
            "service": service,
        ...
        try:
            ...
            # When passthrough_status surfaces an upstream 4xx/5xx the daemon
            # returns the same JSON envelope but with the outer HTTP status
            # set. Re-parse the body so the caller still gets the envelope.
    ...
    def stream(
        ...
        *,
        ...
        """Open a streaming call (server-stream or chunked) and yield SSEEvent.

        The daemon forwards an SSE stream from /api/svc/stream. Iterate until an
        SSEEvent with ``is_terminal=True`` (event in {done, error}).
        """
        req: dict[str, Any] = {
            "peer_id": peer_id,
            "service": service,
        ...
        if body is not None:
            req["body"] = self._encode_body(body)
        # Use a one-shot client so iter_lines is not bounded by the shared
        ...
        with httpx.Client(
            ...
            with c.stream("POST", "/api/svc/stream", json=req) as resp:
                if resp.status_code >= 400:
                    self._check(resp)  # raises SvcAPIError
    ...
    def ws_url(self, name: str) -> str:
        """Return the local ws:// URL clients can connect to.

        Pass it to your favourite WebSocket library:

            import websockets
            async with websockets.connect(svc.ws_url("chat"),
                                          extra_headers={"Authorization": f"Bearer {svc.token}"}) as ws:
                await ws.send(...)
        """
    ...
    def meta(self, name: str) -> Any:
        ...
        otherwise. The probe path was discovered at register time (CP7) when
        ``meta_path`` was given.
        """
    ...
    def audit(self, *, name: str | None = None, limit: int = 50) -> list[dict]:
        ...
        Each row carries: created_at, caller_did, service, method, path, mode,
        status (real upstream HTTP code per CP6), cost, duration_ms, error?.
        """
    ...
    def _encode_body(body: Any) -> Any:
        """Pass-through for JSON-serialisable values; coerce bytes/str to JSON.

        The daemon expects ``body`` to be valid JSON (it stores as RawMessage
        and the upstream handler decides how to decode it). Strings are wrapped
        as JSON strings; bytes are decoded as utf-8 first.
        """
